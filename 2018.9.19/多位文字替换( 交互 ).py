...
def mainReplace( line , otext , etext ): #  使用split()单独处理每一行字符串
    linelist = line.split(otext) 
    result = ''
    for each in intonum( len(linelist) ): # 修改值插入
        linelist.insert( each , etext )
    for i in linelist: # 列表内容,相加转换成字符串
        result += i
    return result

def fontReplace( name , otext , etext): # 主要程序执行步骤
    while True: #   验证文件是否存在
        try:
    ...
    file.close()
    for line in content:
        line_new = mainReplace( line , otext , etext ) # 接受每一行的替换结果
        if count == 1: # 创建新文件写入
...
def findnum( name,otext ): # 使用split()方法查找修改值数量
    while True: # 验证文件是否存在
        try:
            file = open( '%s' % name , 'rt', encoding='utf-8' )
            file.seek(0,0)
            ...
            print("文件不存在 ! ")
            name = input("重新输入: ")
    result = len(file.read().split(otext)) - 1 #    计算需要替换的字符串数量
    return result
